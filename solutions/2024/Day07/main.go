...
func main() {
	...
	for scanner.Scan() {
		...
		task1 := Task1(goal, numbers[0], newArr)
		if task1 {
			task1sum = task1sum + goal
		}
		task2 := Task2(goal, numbers[0], newArr)
		if task2 {
...
func Task1(target, current int, arr []int) bool {
	if len(arr) == 0 {
		//finished
	...
	copy(newArr, arr[1:])
	return Task1(target, current*arr[0], newArr) || Task1(target, current+arr[0], newArr)
}

func Task2(target, current int, arr []int) bool {
	if len(arr) == 0 {
		//finished
		return target == current
	}
	newArr := make([]int, len(arr)-1)
	copy(newArr, arr[1:])
	return Task2(target, current*arr[0], newArr) || Task2(target, current+arr[0], newArr) || Task2(target, concatNums(current, arr[0]), newArr)
}
